...
def test_chunk_text_format(setup_data):
    df_embeddings = setup_data
    assert df_embeddings[EMBEDDING_DF.chunk_text].apply(lambda x: isinstance(x, str) and len(x) > 0).all()


@pytest.mark.skip_this(reason="Skipping test from running because it is calling OpenAI-API")
def test_chunk_embeddings_format(setup_data):
    df_embeddings = setup_data
    assert df_embeddings[EMBEDDING_DF.chunk_embeddings].apply(lambda x: isinstance(x, list) and len(x) == 1536).all()
