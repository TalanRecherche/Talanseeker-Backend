...
def test_normalize_string():
    ...
    assert StringHandler.normalize_string(input_string, remove_special_chars=True) == expected_output


def test_check_similarity_string():
    string_a = "hello world"
    string_b = "hello world"
    assert StringHandler.check_similarity_string(string_a, string_b) == True

    string_a = "hello world"
    string_b = "eating worms"
    assert StringHandler.check_similarity_string(string_a, string_b, threshold=0.9) == False


def test_remove_similar_strings():
    input_strings = ["hello world", "helo wrld", "hello wrld", "goodbye world"]
    expected_output = ["hello world", "goodbye world"]
    assert StringHandler.remove_similar_strings(input_strings, threshold=0.9) == expected_output
