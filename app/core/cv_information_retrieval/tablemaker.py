...
Ideally :
    ...
    2. make the pg_profile table : to each profile, find the collab_id using names/surnames in the pg_collab table.
    3. Now each profile_id is matched to a collab_id
    4. make the pg_cvs table: each cv is assigned a profile_id, which is also assigned a collab_id
    5. etc.
"""
import logging
...
class TableMaker:
    ...
    def _make_pg_chunks(self, df_profiles: pd.DataFrame, df_embeddings: pd.DataFrame) -> pd.DataFrame | None:
        ...
        for idx in range(len(df_profiles)):
            cv_ids = df_profiles.iloc[idx][STRUCTPROFILE_DF.cv_id]
            for cv_id in cv_ids:
                pg_cvs_idx = pg_chunks[(pg_chunks[CHUNK_PG.cv_id] == cv_id)].index.values
                if pg_cvs_idx.size > 0:
                    collab_id = df_profiles.iloc[idx][STRUCTPROFILE_DF.collab_id]
...
if __name__ == "__main__":
    ...
    collab_ids = {files[ii]["file_full_name"]: StringHandler.generate_unique_id(str(ii)) for ii in range(len(files))}
    ...
    DataFrameHandler.save_df(df_embeddings, save_to_file_path=data_path + '\df_embeddings.pkl')
    ...
    DataFrameHandler.save_df(parsed_chunks, save_to_file_path=data_path + '\df_parsed_chunks.pkl')
    print("4", parsed_chunks['collab_id'])
    # =============================================================================
    #     # consolidate CVs
    # =============================================================================
    parsed_chunks = DataFrameHandler.load_df(data_path + '\df_parsed_chunks.pkl')
    ...
    #     # make pg tables
    # =============================================================================
    df_embeddings = DataFrameHandler.load_df(data_path + '\df_embeddings.pkl')
    maker = TableMaker()
