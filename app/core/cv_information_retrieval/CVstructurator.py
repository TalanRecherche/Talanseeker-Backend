...
class CvStructurator:
    ...
    def __init__(self):
        # getting rid of strings that are too long (roles_profile and skills should be short!)
        self.max_string_len = 40
        # similarity threshold to get rid of a string (e.g. data science vs data scientist)
    ...
    def consolidate_cvs(self, df_profile_chunks: pd.DataFrame) -> pd.DataFrame | None:
        ...
        for cv_id, _ in hashmap_profile_filename.items():
            ...
            df_filtered_profile = df_profile_chunks[df_profile_chunks[STRUCTCV_DF.cv_id] == cv_id]
            ...
            if not df_single_profile.empty:
                df_consolidated_cvs = pd.concat(
                    [df_consolidated_cvs, df_single_profile], axis=0, ignore_index=True)
    ...
    def _consolidate_single_cv(self, df_chunks_profile: pd.DataFrame) -> pd.DataFrame:
        ...
        for column in STRUCTCV_DF.static_columns_:
            hashmap_single_cv[column] = self._merge_static_columns(column, df_chunks_profile)

        for column in STRUCTCV_DF.numerical_columns_:
            hashmap_single_cv[column] = self._merge_numerical_columns(column, df_chunks_profile)
    ...
    def _merge_numerical_columns(self, column, df_chunks_profile) -> int:
        ...
        try:
            numerical_extract = df_chunks_profile[column].dropna().apply(self._extract_integer)
            numerical_value = int(max(numerical_extract))
        except Exception as e:
            numerical_value = 0
            logging.warning("CVStructurator : numerical columns structuration failed {}".format(e))
    ...
    def _merge_string_columns(self, column: str, df_chunks_profile: pd.DataFrame) -> list[str]:
        """
        ...
        try:
            ...
            if column != STRUCTCV_DF.diplomas_certifications:
                ...
                values = [value for value in values if len(value) <= self.max_string_len]
            # 5 fuzzy string matching to remove pseudo-duplicates (this is O(n2),
            # a C++ version exists see Cdiflib)
            values = StringHandler.remove_similar_strings(values, threshold=self.similarity_threshold)
        except Exception as e:
            values = []
            logging.warning("CVStructurator : string columns structuration failed: {}".format(e))
        return values
    ...
    def _get_unique_cvs(self, df_profile_chunks: pd.DataFrame) -> dict:
        ...
        """
        unique_cvs = set(ListHandler.flatten_list(list(df_profile_chunks[PARSED_DF.cv_id])))
        hashmap_cv_chunks = {}

        for cv in unique_cvs:
            filtered_df_profile_chunks = df_profile_chunks[df_profile_chunks[PARSED_DF.cv_id] == cv]
            hashmap_cv_chunks[cv] = list(filtered_df_profile_chunks[PARSED_DF.chunk_id].values)
