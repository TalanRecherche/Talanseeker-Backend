...
The class methods are:
    ...
    - validate_dataframe(df): check if the dataframe is valid according to the schema (check value type and columns name)
...
class ParentPandasModel:
    ...
    def generate_dataframe(cls) -> pd.DataFrame:
        """
        Generate an empty dataframe with the columns of the schema and correct data types
        Args:
        ...
        empty_data = {col_name: pd.Series([], dtype=_pandera_dtype_to_pandas_dtype(col.dtype)) for col_name, col in
                      cls.schema.columns.items()}
