...
class AzurePGManager:
    ...
    def _sql_req_creator(req_target: str, **kwargs) -> str:
        ...
        if req_target == "collab_by_id":
            ...
            req = (select(text(kwargs["table_name"])).
                   where(PgChunks.collab_id == kwargs["collab_id"] &
                         PgChunks.chunk_id == kwargs["collab_id"]))
    ...
    def upsert(row: pd.Series, table_name: str, policy: str) -> None:
        ...
        if policy == UpsertPolicies.PROFILE_UPDATE:
            ...
            if len(data) == 0:
                ...
                req = AzurePGManager._sql_req_creator("collab_by_id",
                                                  table_name = PG_Profiles.__tablename__,
                                                  collab_id = row[ProfilePg.collab_id])
                with Session(engine) as session:
                    session.execute(req)
                    session.commit()
            ...
            req = AzurePGManager._sql_req_creator("cv_chunks",
                                                  table_name=PgChunks.__tablename__,
                                                  collab_id=row[PgChunks.collab_id],
