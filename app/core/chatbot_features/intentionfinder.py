"""This module is used to process the user query and extract the intention of the user."""
import logging
import pandas as pd
...
class IntentionFinder:
    """
    Class used to process the user query and extract the intention of the user.
    It uses prompt engineering to call the llm and extract the information from the output.

    Requires:
        - GUESSINTENTION__QUERY_EXAMPLES_PATH environment variable to be set to the path of the
            file containing the query examples. Set it in .env file.
        - GUESSINTENTION__SYSTEM_TEMPLATE_PATH environment variable to be set to the path of the
    ...
    def __init__(self, settings: Settings):
        ...
        self.query_samples = self.settings.guess_intention_settings.guess_intention_query_template
        self.system_template = self.settings.guess_intention_settings.guess_intention_system_template
        self.roleseeker_system_template = self.settings.guess_intention_settings.roleseeker_template
        self.roleseeker_query_samples = self.settings.guess_intention_settings.roleseeker_query_template
    ...
    def guess_intention(self, user_query: str, _format: str = 'dataframe'):
        ...
        Args:
            ...
            _format (str, optional): The format preferred for the output. Defaults to 'dataframe'.

        Returns:
            format: Result of the process in the format specified. Defaults to 'dataframe'.
        """
    ...
    def _prepare_roleseeker_template(self, uquery: str):
        ...
        """
        uquery_template = self.roleseeker_system_template. \
            replace('{exemples}', self.roleseeker_query_samples). \
    ...
    def _process_roleseeker_answer(self, output_llm: str):
        ...
        assert len(ans[QUERY_STRUCT.roles]) == len(ans[QUERY_STRUCT.nb_profiles]), \
            f"roles and nb_profiles have different lengths: \
                {ans[QUERY_STRUCT.roles]} and {ans[QUERY_STRUCT.nb_profiles]}"
        return ans

    def _extract_infos_per_role(self, user_query: str,
                                _format: str,
                                roles: dict):
        ...
        """
        res = []
        for role, n_role in zip(roles[QUERY_STRUCT.roles],
        ...
        if _format == 'dataframe':
            ...
            raise NotImplementedError(f'format {_format} not implemented. Use dataframe.')
        return out

    def _extract_text_from_llmoutput(self, output_llm: str, field: str):
        """Extracts the value associated to the field to extract from the output of the llm.
        ...
        try:
            ...
            logging.error("llm output not as expected: %s.\n%s(field: %s)\n%s",
                          output_llm, exc, field, output_llm. \
                          split(f"{field} : "))
            res = 'Non renseigné'
        return res
    ...
    def _prepare_template(self, uquery: str, role: str):
        ...
        Returns:
            str: Complete prompt to use to call the llm.
        """
        uquery_template = self.system_template. \
            replace('{exemples}', self.query_samples). \
            replace('{query}', uquery). \
            replace('{role}', role)
        return uquery_template
    ...
    def _process_answer(self, output_llm: str, uquery: str, role: str, n_role: int):
        ...
        Example:
            ...
            'user_query': 'Trouve moi une équipe de développeurs pour une mission banque',
            'years': ['Non renseigné']}
        ...
        ans[QUERY_STRUCT.simplified_query] = self._wrap_txt_with_list(
            self._extract_text_from_llmoutput(
                output_llm, "simplified_mission"))
        ...
        ans[QUERY_STRUCT.years] = StringHandler.string_to_list_with_separator(
            ...
            separator=',')
        ans[QUERY_STRUCT.diplomas_certifications] = StringHandler.string_to_list_with_separator(
        ...
        ans[QUERY_STRUCT.soft_skills] = StringHandler.string_to_list_with_separator(
            ...
            separator=";")
        ans[QUERY_STRUCT.technical_skills] = StringHandler.string_to_list_with_separator(
            self._extract_text_from_llmoutput(output_llm, "Compétences techniques"),
            separator=";")
        ans[QUERY_STRUCT.roles] = [role]
        ans[QUERY_STRUCT.missions] = self._wrap_txt_with_list(self._extract_text_from_llmoutput(
            output_llm, "Objectif mission"))
        ans[QUERY_STRUCT.sectors] = self._wrap_txt_with_list(self._extract_text_from_llmoutput(
            output_llm, "Secteurs d'activité"))
        ans[QUERY_STRUCT.companies] = self._wrap_txt_with_list(self._extract_text_from_llmoutput(
            output_llm, "Compagnie"))
        return ans
