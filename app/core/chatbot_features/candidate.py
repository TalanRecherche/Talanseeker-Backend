...
class Candidates:
    def __init__(self, pg_chunks: pd.DataFrame,
                 pg_collabs: pd.DataFrame,
                 pg_cvs: pd.DataFrame,
                 pg_profiles: pd.DataFrame) -> None:
        ...
        for idx, row in pg_collabs.iterrows():
            ...
            rows_pg_chunks = pg_chunks[pg_chunks[CHUNK_PG.collab_id] == current_collab_id]
            rows_pg_collabs = pg_collabs[pg_collabs[COLLAB_PG.collab_id] == current_collab_id]
            rows_pg_cvs = pg_cvs[pg_cvs[CV_PG.collab_id] == current_collab_id]
            rows_pg_profiles = pg_profiles[pg_profiles[PROFILE_PG.collab_id] == current_collab_id]
            # instantiate a candidate
            candidate = Candidate(rows_pg_chunks,
                                  rows_pg_collabs,
                                  rows_pg_cvs,
                                  rows_pg_profiles)
...
class Candidate:
    def __init__(self, row_pg_chunks: pd.DataFrame,
                 row_pg_collabs: pd.DataFrame,
                 row_pg_cvs: pd.DataFrame,
                 row_pg_profiles: pd.DataFrame) -> None:
        # from COLLAB table
        self.name = self.get_value_or_default(row_pg_collabs, COLLAB_PG.name).title()
        self.surname = self.get_value_or_default(row_pg_collabs, COLLAB_PG.surname).upper()
        ...
        self.sub_role = self.get_value_or_default(row_pg_collabs, COLLAB_PG.sub_role).title()
        self.grade = self.get_value_or_default(row_pg_collabs, COLLAB_PG.grade).title()
        self.region = self.get_value_or_default(row_pg_collabs, COLLAB_PG.region).title()
        self.city = self.get_value_or_default(row_pg_collabs, COLLAB_PG.city).title()
        self.manager = self.get_value_or_default(row_pg_collabs, COLLAB_PG.manager).title()

        # from PROFILE table
        self.years_of_exp = self.get_value_or_default(row_pg_profiles, PROFILE_PG.years)
        self.roles_profile = ListHandler.capitalize_list(self.get_value_or_default(row_pg_profiles, PROFILE_PG.roles, value_is_list=True))
        self.soft_skills = ListHandler.capitalize_list(self.get_value_or_default(row_pg_profiles, PROFILE_PG.soft_skills, value_is_list=True))
        self.technical_skills = ListHandler.capitalize_list(self.get_value_or_default(row_pg_profiles, PROFILE_PG.technical_skills, value_is_list=True))
        self.skills = self.technical_skills + self.soft_skills
        self.diplomas_certifications = ListHandler.capitalize_list(self.get_value_or_default(row_pg_profiles, PROFILE_PG.diplomas_certifications, value_is_list=True))
    ...
    def get_value_or_default(self, row, column, default_value='', value_is_list=False):
        """
        Get the value from the row for the given column, or return the default value if it's null.
        ...
        :return: The value from the row for the given column, or the default value if it's null.
        """
