...
def evaluate_response(expected, actual):
    ...
    score['completeness'] = (expected_properties / expected_properties) * 20  # Total completeness score is 20
    ...
    if actual_roles_count > expected_roles_count:
        penalty = (actual_roles_count - expected_roles_count) / actual_roles_count * 20  # Penalty weight can be adjusted
    ...
    score['similarity'] = (similarity_score / ((len(expected_roles) * 5) + 2)) * 60  # 5 criteria per role + 2 (sectors and companies), total similarity score is 60

    score['total_score'] = score['completeness'] + score['similarity'] + score['syntax']
    return score

@observe()
def test_01_structured_query_format(request: any) -> None:
    global settings
...
if __name__ == "__main__":
    ...
    for idx, request in enumerate(data.get("requests")):
        test_01_structured_query_format(request)
